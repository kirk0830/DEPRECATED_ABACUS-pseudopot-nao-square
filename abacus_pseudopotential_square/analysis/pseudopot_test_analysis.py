...
"""
All folders are named in format t_[pseudopot_kind]_[version](_[appendix])_[system],
and the system is, relatively fixed. Currently only nspin = 1 is supported.

Density Of States (DOS) will be calculated for each folder, and the DOS will be plotted.
Below fermi level all bands are filled and colored in blue, and above fermi level all bands 
are empty and uncolored.

Band gaps are noted at topright corner of each DOS plot and, singly drawn again in bar plot.
"""

def read_energies(tests: dict) -> dict:

    print("reading energies from scf.log of each test...")
    log_status = {}
    root = tests["work_folder"]
    if root[-1] != "/":
        root += "/"
    for system in tests["systems"].keys():
        log_status[system] = {}
        for version in tests["versions"]:
            for appendix in tests["appendices"]:
                for pseudopot_kind in tests["pseudopot_kinds"]:
                    if appendix == "":
                        folder_name = "t_"+pseudopot_kind+"_"+version+"_"+system
                    else:
                        folder_name = "t_"+pseudopot_kind+"_"+version+"_"+appendix+"_"+system
                    #if folder name exists
                    folder_name = root + folder_name
                    if os.path.isdir(folder_name):
                        os.chdir(folder_name)
                        print("enter folder ", folder_name)
                        if os.path.exists("CRASH"):
                            print("CRASH detected in folder ", folder_name)
                            os.chdir("..")
                            print("back to folder ", os.getcwd())
                            continue
                        test = folder_name.split("/")[-1]
                        log_status[system][test] = {
                            "pseudopotential": {
                                "pseudopot_kind": pseudopot_kind,
                                "version": version,
                                "appendix": appendix
                            },
                            "dos": {
                                "window_width": tests["dos"]["window_width"],
                                "emin": -1, "emax": -1,
                                "delta_e": tests["dos"]["delta_e"],
                                "smear": tests["dos"]["smear"]
                            },
                            "energies": {
...
def statistics(test_status: dict, log_status: dict) -> None:

    print("To do precision statistics, experimental values are needed for all systems.")
    ncol = 2
    fig, axs = plt.subplots(int(np.ceil(len(test_status["systems"].keys())/ncol)), ncol, figsize=(8, 8), squeeze=False)
    
    add_reference_line = False
    num_reference_values = 0

    for system in test_status["systems"].keys():
        if test_status["systems"][system]["experimental_value"] >= 0:
    ...
    for icol, system in enumerate(log_status.keys()):
        ...
        _icol = icol%ncol
        if add_reference_line:
            axs[_irow][_icol].hlines(y=test_status["systems"][system]["experimental_value"], xmin=-0.5, xmax=len(band_gaps[-1])+0.5, linestyle='--', color = 'red')
            axs[_irow][_icol].text(len(band_gaps[-1])+1.0, test_status["systems"][system]["experimental_value"], "Experimental value", ha='left', va='center', color='red', fontsize=12)
            # readjust to ensure reference line always echo
            axs[_irow][_icol].set_ylim(bottom = 0, top = max(max(band_gaps[-1]), test_status["systems"][system]["experimental_value"]) + 0.5)
...
def main(tests: dict, test_dimension = 1) -> None:

    print("enter main function, current working directory is ", os.getcwd(), ". \nAccording to user's set, the working directory is ", tests["work_folder"], ".")
    tests["work_folder"] = os.path.abspath(tests["work_folder"])
    """ test_dimension = 1 or 2 """
    if test_dimension == 1:
        log_status = read_energies(tests)
        if tests["dos"]["draw"]:
            log_status = calculate_dos(log_status, delta_e = tests["dos"]["delta_e"], smear = tests["dos"]["smear"])
            draw_dos(log_status)
        if tests["draw_statistics"]:
            statistics(tests, log_status)
    elif test_dimension == 2:
        exist_tests_1d_startswith, tests_2d, mapping_tests_2d_to_1d = preprocess_2d(tests)
        log_status = read_energies(tests_2d)
        data = postprocess_2d(exist_tests_1d_startswith, tests_2d, mapping_tests_2d_to_1d, log_status)
        if tests["plot_band_gap_2d"]["draw"]:
            plot_band_gap_2d(
                mode = tests["plot_band_gap_2d"]["mode"],
                data = data, 
                ncol = 2,
                color_map = tests["plot_band_gap_2d"]["color_map"])
...
if __name__ == "__main__":
    ...
    tests = {
        ...
        "pseudopot_kinds": ["dojo", "sg15", "rrkjus", "kjpaw", "hgh", "pd", "pslnc"],
        "versions": ["03", "04", "05", "1", "2", "10", "11", "12", "031", "100"],
        "appendices": ["", "d", "fr", "spd"],
        "systems": {
            "InN": {
                "experimental_value": 0.74,
            },
            "InP": {
                "experimental_value": 1.34,
            },
            "InAs": {
                "experimental_value": 0.41,
            },
            "InSb": {
                "experimental_value": 0.23,
            },
        }
    }
    main(tests, 2)
