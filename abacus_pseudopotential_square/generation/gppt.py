    ...
    for functional in test_status["calculation"]["functionals"]:
        for system in test_status["systems"].keys():
            for test in test_status["systems"][system].keys():
                ...
                os.system("sed -i 's/ecut_to_test/" + str(test_status["systems"][system][test]["ecutwfc"]) + "/g' " + folder + "\\scf.in")
    ...
    for functional in test_status["calculation"]["functionals"]:
        for system in test_status["systems"].keys():
            for test in test_status["systems"][system].keys():
                ...
                # adjust ecutwfc in input file
                os.system("sed -i 's/ecut_to_test/" + str(test_status["systems"][system][test]["ecutwfc"]) + "/g' " + folder + "\\INPUT")
                # copy pseudopotential to test folder
                for element in test_status["systems"][system][test]["elements"]:
                    pseudopot_path = test_status["paths"]["pseudopotential"]["resources"] + "\\"
                    pseudopot_path += test_status["systems"][system][test]["pseudopotentials"]["info"][element]["kind"]
